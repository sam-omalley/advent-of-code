...
pub fn parse(input: &str) -> Input {
    ...
    for interval in input.split(",").map(|x| x.trim()).filter(|x| !x.is_empty()) {
        let (a, b) = interval.split_once("-").unwrap();

        res.push((a.parse().unwrap(), b.parse().unwrap()))
    }
...
pub fn part2(input: &Input) -> u64 {
    ...
    for (a, b) in input {
        total += check_all_naughty_patterns(*a, *b);
    }
